...
const Requete = {
  Login: function (db, req, res) {
    ...
    bcrypt.hash(password, saltRounds, function (err, hash) {
      db.query(
        "SELECT * FROM users WHERE Email=? Or Phone=? AND Mdp=? ",
        [email, phone, hash],
    ...
    bcrypt.hash(Mdp, saltRounds, function (err, hash) {
      db.query(
        ...
        (error, results, fields) => {
          ...
          if (results.length == 0) {
            db.query(
              "INSERT INTO users (Nom,Prenom,Phone,Mdp,Email,DateCreation,TypeUser,EtatCompte) VALUES (?,?,?,?,?,?,?,?) ",
              [
                Nom,
                Prenom,
                Phone,
